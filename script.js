...
function preprocessText(text) {
  ...
  // ── Protect tables before newline expansion ──
  // GFM tables require consecutive rows with no blank lines between them.
  // Extract them first, then restore after newline doubling.
  const tableBlocks = [];
  let protected_text = text;

  // Match GFM tables: header | alignment | rows
  // Pattern: line with |...|, followed by |---|...|, followed by | ... | rows
  protected_text = protected_text.replace(
    /(\|.+\|[ \t]*\n)(\|[ \t]*[\-:| ]+\|[ \t]*\n)((?:\|.+\|[ \t]*(?:\n|$))+)/gm,
    (match) => {
      const id = `TABLEBLOCK${tableBlocks.length}ENDTABLE`;
      tableBlocks.push({ id, content: match });
      return id;
    },
  );
  ...
  // We do this AFTER protecting tables so table rows stay consecutive.
  protected_text = protected_text.replace(/([^\n])\n([^\n])/g, "$1\n\n$2");
  ...
  tableBlocks.forEach(({ id, content }) => {
    protected_text = protected_text.split(id).join(content);
  });
...
function clearAll() {
  inputEl.value = "";
  previewEl.innerHTML =
    '<p class="placeholder-hint">Rendered output will appear here.</p>';
  statusBar.className = "status-bar hidden";
}
